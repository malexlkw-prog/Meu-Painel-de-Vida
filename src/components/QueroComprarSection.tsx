...
const CATEGORIES = [
  { id: 'clothes', name: 'Roupas', icon: Shirt, desc: 'Camisas, camisas de time, calças, shorts e casacos.', color: 'text-indigo-500 bg-indigo-50 dark:bg-indigo-950/40', defaultSubs: ['Camisas', 'Camisas de Futebol', 'Calças', 'Shorts', 'Jaquetas', 'Camisas Sociais', 'Bonés', 'Meias'] },
...
export default function QueroComprarSection({ data, onUpdateData, onClose }: QueroComprarSectionProps) {
  ...
  const itemsToDisplay = useMemo(() => {
    ...
    if (view === 'category' && selectedCategoryId) {
      ...
      // Extra subcategory filter for clothes
      if (selectedCategoryId === 'clothes' && clothingSubCategory !== 'all') {
    ...
    return list;
  }, [state.items, view, selectedCategoryId, clothingSubCategory, selectedPersonId, searchQuery, statusFilter, priorityFilter, storeFilter, priceSort]);
  ...
  const mergedCategories = useMemo(() => {
    ...
    return [...mainCats, ...mappedCustoms, ...footerCats];
          ...
          <div className="space-y-6 text-left">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-[#111726] p-5 border border-slate-150 dark:border-slate-850 rounded-3xl shadow-3xs">
              ...
              <div className="flex items-center gap-2">
                ...
                {view === 'category' && selectedCategoryId === 'clothes' && (
                  <button
                    onClick={() => setShowAddSubCatModal(true)}
                    className="text-slate-400 hover:text-indigo-600 dark:text-slate-500 dark:hover:text-indigo-400 text-xs font-semibold px-2.5 py-2 rounded-lg flex items-center gap-1 transition-all hover:bg-slate-100 dark:hover:bg-slate-900/60 cursor-pointer"
                    title="Nova Subcategoria"
                  >
                    <Plus size={12} /> <span className="underline decoration-dotted underline-offset-4">Nova Subcategoria</span>
                  </button>
                )}
              </div>
            </div>

            {/* Subtabs for clothing */}
            {view === 'category' && selectedCategoryId === 'clothes' && (
              <div className="flex flex-wrap gap-1.5 bg-slate-100 dark:bg-[#111726]/40 p-1.5 rounded-2xl border border-slate-200/50 dark:border-slate-850 overflow-x-auto">
                <button
                ...
                {clothingSubcategories.map(sub => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wide transition-all ${clothingSubCategory === sub ? 'bg-white dark:bg-[#111726] text-pink-650 dark:text-pink-400 shadow-3xs' : 'text-slate-500 hover:text-slate-850 dark:hover:text-white'}`}
                  >
                    {sub}
                  </button>
                ))}
